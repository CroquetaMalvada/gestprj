...
def login_view(request):
    if request.POST:
        ...
        if user is not None and (user.groups.filter(name="Admins gestprj").exists() or user.groups.filter(name="Investigadors Principals").exists()):#si el usuario forma parte de alguno de los grupos necesarios y esta en el creaf
            login(request, user)
            # tipo = LDAPGroupType("cn=Users").is_member(user,"Investigadors Principals")
            # tipo = user.groups.filter(name='Investigadors Principals').exists()
            # grupos = user.ldap_user.group_names
